...
# install rclone
ARG RCLONE_VERSION
RUN debfile="/tmp/rclone.deb" && curl -fsSL -o "${debfile}" "https://downloads.rclone.org/v${RCLONE_VERSION}/rclone-v${RCLONE_VERSION}-linux-amd64.deb" && dpkg --install "${debfile}" && rm "${debfile}"
